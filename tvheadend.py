...
def configureTvheadend(playlist, outdir):
    ...
    for entry in playlist:
        ...
        channel.write(json.dumps({'name': entry.title, 'channel_number': id}, ensure_ascii = False, indent = 4, separators = (',', ': ')))
        channel.close()
        service = open(outdir + '/iptvservices/iptv_' + str(id), 'w')
        group, port = entry.path.split('udp://@')[1].split(':', 1)
        service.write(json.dumps({'pmt':0, 'stype': 1, 'pcr': 0, 'port': port, 'group': group, 'interface': 'eth2', 'channelname': entry.title, 'mapped': 1, 'disabled': 0}, ensure_ascii = False, indent = 4, separators = (',', ': ')))
        service.close()
